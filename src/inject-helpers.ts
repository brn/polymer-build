...
export type Helpers = 'typescript' | 'babel';


export const helpers = {
  TS: 'typescript' as 'typescript',
  BABEL: 'babel' as 'babel'
};


/**
 * When transform script, if we need to inject transpiler helper,
 * this class inject any helper's that prepared.
 */
export class HelpersInjector extends AsyncTransformStream<File, File> {
  constructor(private entrypoint: string, private helperName: Helpers) {
  ...
  private async processFile(file: File): Promise<File> {
    ...
    dom5.setTextContent(
        ...
        await fs.readFile(
            path.join(__dirname, `${this.helperName}-helpers.min.js`), 'utf-8'));
